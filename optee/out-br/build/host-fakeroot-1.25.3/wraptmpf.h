...
int tmp_posix_spawn (pid_t * __restrict pid, const char * __restrict path, const posix_spawn_file_actions_t *file_actions, const posix_spawnattr_t * __restrict attrp, char *const argv[ __restrict], char *const envp[ __restrict]){
  load_library_symbols();
  return  next_posix_spawn (pid, path, file_actions, attrp, argv, envp);
}

int tmp_posix_spawnp (pid_t * __restrict pid, const char * __restrict path, const posix_spawn_file_actions_t *file_actions, const posix_spawnattr_t * __restrict attrp, char *const argv[ __restrict], char *const envp[ __restrict]){
  load_library_symbols();
  return  next_posix_spawnp (pid, path, file_actions, attrp, argv, envp);
}

#endif
int tmp_execve (const char *path, char *const argv[], char *const envp[]){
  load_library_symbols();
  return  next_execve (path, argv, envp);
}

int tmp_execv (const char *path, char *const argv[]){
  load_library_symbols();
  return  next_execv (path, argv);
}

int tmp_execvp (const char *file, char *const argv[]){
  load_library_symbols();
  return  next_execvp (file, argv);
}

int tmp_execvP (const char *file, const char *search_path, char *const argv[]){
  load_library_symbols();
  return  next_execvP (file, search_path, argv);
}
...
int tmp_getgroups (int size, gid_t list[]){
  load_library_symbols();
  return  next_getgroups (size, list);
}
...
int tmp_setpriority (int which, int who, int prio){
  load_library_symbols();
...
int tmp_capset (cap_user_header_t hdrp, const cap_user_data_t datap){
  load_library_symbols();
  return  next_capset (hdrp, datap);
}

#endif /* HAVE_CAPSET */
#ifdef HAVE_LISTXATTR
 ssize_t tmp_listxattr (const char *path, char *list, size_t size){
  load_library_symbols();
 ...
 ssize_t tmp_llistxattr (const char *path, char *list, size_t size){
  load_library_symbols();
 ...
 ssize_t tmp_flistxattr (int fd, char *list, size_t size){
  load_library_symbols();
...
#ifdef HAVE_GETXATTR
ssize_t tmp_getxattr (const char *path, const char *name, void *value, size_t size){
  load_library_symbols();
...
ssize_t tmp_lgetxattr (const char *path, const char *name, void *value, size_t size){
  load_library_symbols();
...
ssize_t tmp_fgetxattr (int fd, const char *name, void *value, size_t size){
  load_library_symbols();
...
#ifdef HAVE_SETXATTR
ssize_t tmp_setxattr (const char *path, const char *name, void *value, size_t size, int flags){
  load_library_symbols();
...
ssize_t tmp_lsetxattr (const char *path, const char *name, void *value, size_t size, int flags){
  load_library_symbols();
...
ssize_t tmp_fsetxattr (int fd, const char *name, void *value, size_t size, int flags){
  load_library_symbols();
...
ssize_t tmp_removexattr (const char *path, const char *name){
  load_library_symbols();
...
ssize_t tmp_lremovexattr (const char *path, const char *name){
  load_library_symbols();
...
ssize_t tmp_fremovexattr (int fd, const char *name){
  load_library_symbols();
...
acl_t tmp_acl_get_fd (int fd){
  load_library_symbols();
  return  next_acl_get_fd (fd);
}

acl_t tmp_acl_get_file (const char *path_p, acl_type_t type){
  load_library_symbols();
  return  next_acl_get_file (path_p, type);
}
...
int tmp_acl_get (const char *path, int flags, acl_t **aclp){
  load_library_symbols();
  return  next_acl_get (path, flags, aclp);
}

int tmp_facl_get (int fd, int flags, acl_t **aclp){
  load_library_symbols();
  return  next_facl_get (fd, flags, aclp);
}

int tmp_acl_set (const char *path, acl_t *aclp){
  load_library_symbols();
  return  next_acl_set (path, aclp);
}

int tmp_facl_set (int fd, acl_t *aclp){
  load_library_symbols();
  return  next_facl_set (fd, aclp);
}

int tmp_acl_trivial (const char *path){
  load_library_symbols();
  return  next_acl_trivial (path);
}

int tmp_acl (const char *path, int cmd, int cnt, void *buf){
  load_library_symbols();
  return  next_acl (path, cmd, cnt, buf);
}

int tmp_facl (int fd, int cmd, int cnt, void *buf){
  load_library_symbols();
  return  next_facl (fd, cmd, cnt, buf);
}
...
#ifdef HAVE_STATX
int tmp_statx (int dir_fd, const char *pathname, int flags, unsigned int mask, struct statx *buf){
  load_library_symbols();
  return  next_statx (dir_fd, pathname, flags, mask, buf);
}

#endif /* HAVE_STATX */

#ifdef __sun
int tmp_sysinfo (int command, char *buf, long count){
  load_library_symbols();

...
MPERS_PRINTER_DECL(_Bool, fetch_bpf_fprog, struct tcb *const tcp, const kernel_ulong_t addr, void *const p)
{
 struct bpf_fprog *pfp = p;
 struct_sock_fprog mfp;
 if ((sizeof(*pfp) == sizeof(mfp))
     && (__builtin_offsetof (struct bpf_fprog, filter) ==
  __builtin_offsetof (struct_sock_fprog, filter)))
  return !umoven_or_printaddr((tcp), (addr), sizeof(*(pfp)), (void *) (pfp));
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&mfp)), (void *) (&mfp)))
  return 0;
 pfp->len = mfp.len;
 pfp->filter =
  (uintptr_t)

...
void tlb_init(CPUState *cpu)
{
	...
	//	 # KWON 	  2021.11.17 23:39:41  
	// c는 CPUTLBCommon 구조체의 인스턴스

	//		 # KWON 	  2021.11.18 17:47:12  
	fprintf(stderr,"target_ulong = uint32_t\n");
	fprintf(stderr,"target_long = int32_t\n");

	fprintf(stderr,"TARGET_PAGE_BITS = %d\n",TARGET_PAGE_BITS);
	fprintf(stderr,"TARGET_LONG_SIZE = %d\n",TARGET_LONG_SIZE);
	fprintf(stderr,"TARGET_LONG_BITS = %d\n", TARGET_LONG_BITS);
	fprintf(stderr,"TARGET_PHYS_ADDR_SPACE_BITS = %d\n",TARGET_PHYS_ADDR_SPACE_BITS);
	fprintf(stderr,"TARGET_VIRT_ADDR_SPACE_BITS =  %d\n",TARGET_VIRT_ADDR_SPACE_BITS);
	fprintf(stderr,"CPU_VTLB_SIZE = %d\n",CPU_VTLB_SIZE);
	fprintf(stderr,"CPU_TLB_ENTRY_BITS = %d\n",CPU_TLB_ENTRY_BITS);
	fprintf(stderr,"CPU_TLB_DYN_MIN_BITS = %d\n",CPU_TLB_DYN_MIN_BITS);
	fprintf(stderr,"CPU_TLB_DYN_DEFAULT_BITS = %d\n",CPU_TLB_DYN_DEFAULT_BITS);
	fprintf(stderr,"CPU_TLB_DYN_MAX_BITS =  %d\n",CPU_TLB_DYN_MAX_BITS);
	
	fprintf(stderr,"HOST_LONG_BITS = %d\n",HOST_LONG_BITS);
	fprintf(stderr,"NB_MMU_MODES = %d\n\n", NB_MMU_MODES);
/*
	for(int i=0;i<CPU_VTLB_SIZE;i++)
	{
		fprintf(stderr,"%u \n",env_tlb(env)->d[i].vtable->addr_read);
		fprintf(stderr,"%u \n",env_tlb(env)->d[i].vtable->addr_write);
		fprintf(stderr,"%u \n",env_tlb(env)->d[i].vtable->addr_code);
		fprintf(stderr,"%lu \n",env_tlb(env)->d[i].vtable->addend);
...
*/
	...
	/*
	for(int i=0;i<sizeof(env_tlb(env)->d);i++)
	{
		fprintf(stderr,env_tlb(env)->f[i]);
	}
	*/
...
{
	//	 # KWON 	  2021.11.17 22:59:54  
    tlb_debug("mmu_idx: 0x%" PRIx16 "\n", idxmap);
	//fprintf(stderr, "mmu_idx: 0x%" PRIx16 "\n", idxmap);
...
{
    //KWON
	//fprintf(stderr,"/accel/tcg/cputlb.c\n");
                                        ...
                                        void **hostp)
{
	...
	for(int i=0;i<CPU_VTLB_SIZE;i++)
	{
		fprintf(stderr,"| %d.addr_read = 0x%x",i,env_tlb(env)->f[i].table->addr_read);
		fprintf(stderr,"| %d.addr_write = 0x%x",i,env_tlb(env)->f[i].table->addr_write);
		fprintf(stderr,"| %d.addr_code = 0x%x",i,env_tlb(env)->f[i].table->addr_code);
		fprintf(stderr,"| %d.addend = 0x%ld |\n",i,env_tlb(env)->f[i].table->addend);
		fprintf(stderr,"--------------------------------------------------------------");
		fprintf(stderr,"-----------------------------------------------");
	fprintf(stderr,"\n");
